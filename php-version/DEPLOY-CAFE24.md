...
기존 설치에는 새 코드를 서비스하기 전에 동일한 DB 설정으로 아래 명령을 한 번 실행합니다. `MNM_CONFIG`가 있다면 대상 서버의 설정 파일을 가리키도록 지정합니다.

```sh
php mnm-private/tools/migrate-card-receipts.php
```

로컬 소스 구조에서는 `php php-version/tools/migrate-card-receipts.php`를 사용합니다. 이 명령은 `CardExpense.receiptAttachments` nullable 텍스트 컬럼만 추가하며 재실행할 수 있습니다. 기존 첫 증빙의 경로·이름·파일은 유지됩니다. 신규 설치에는 기본 스키마에 포함되어 있습니다. 웹 요청에서는 스키마 변경을 실행하지 않습니다.

추가 파일의 목록도 검토 토큰에 포함되어 오래된 승인 요청을 차단합니다. DB와 증빙 폴더를 함께 백업해야 하며 내보내기 도구는 첫 증빙과 추가 증빙을 모두 포함합니다. 이전 교체 동작으로 이미 삭제된 파일은 이 업데이트로 복원되지 않습니다.
