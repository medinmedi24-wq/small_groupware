...
try {
    ...
    if($page==='setup'){ob_start();try{echo '<section class="setup panel formpanel">';setupPage();echo '</section>';$content=ob_get_clean();}catch(Throwable $e){ob_end_clean();throw $e;}}
        ...
        if($u&&$page==='receipt')downloadReceipt($u,(string)($_GET['id']??''),isset($_GET['attachment'])?(string)$_GET['attachment']:null);
...
$inlineLeaveError=$error&&$_SERVER['REQUEST_METHOD']==='POST'&&in_array($_POST['action']??'',['leave_create','leave_edit'],true)&&str_contains($content,'<!-- leave-submit-error -->');
if($inlineLeaveError)$content=str_replace('<!-- leave-submit-error -->','<div class="error" id="leave-submit-error" role="alert" tabindex="-1">'.e($error).'</div>',$content);
?><!doctype html>
<html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#2b4390"><meta name="apple-mobile-web-app-title" content="메디앤메디"><title>메디앤메디</title><link rel="manifest" href="manifest.webmanifest?v=2"><link rel="apple-touch-icon" href="icons/groupware-180.png?v=2"><link rel="stylesheet" href="assets/style.css"><link rel="stylesheet" href="assets/admin.css"><link rel="stylesheet" href="assets/icons.css"><link rel="stylesheet" href="assets/php.css?v=28"><link rel="stylesheet" href="assets/branding.css?v=3"><script src="assets/splash.js?v=3"></script><script src="assets/app.js?v=8" defer></script><script src="assets/attendance-gps.js?v=4" defer></script><script src="assets/attendance-sync.js?v=1" defer></script></head><body>
...
<?php if($error&&!$inlineLeaveError): ?><div class="error" role="alert"><?=e($error)?></div><?php endif; ?>
